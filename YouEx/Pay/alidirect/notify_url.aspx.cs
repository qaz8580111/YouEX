    ...
    public partial class notify_url : Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //start测试代码----------------------------------------------------------
                //foreach (KeyValuePair<string, string> dict in sArrary)
                //{
                //    File.AppendAllText("D:\\www\\DD", string.Format("{0}={1} \r\n", dict.Key, dict.Value));
                //}
                ...
                if (sArrary.Count > 0)//判断是否有带返回参数
                {
                    ...
                    if (responseTxt == "true" && sign == mysign)//验证成功
                    {
                        ...
                        // int order_no = MAJRequest.GetInt("out_trade_no", 0);     //获取订单号
                        ...
                        if (Request.Form["trade_status"] == "TRADE_FINISHED" || Request.Form["trade_status"] == "TRADE_SUCCESS")
                        {
                            ...
                            //  AcountInfo ai = uc.GetAcountById(order_no);
                            //拆解订单号信息 【0】格式（FD_）【1】支付单编号【2】订单号
                            string[] orderNo = order_no.Split('_');
                            int orderId = -1;

                            //判断状态交易单号是否存在 或 已支付
                            if (Int32.TryParse(orderNo[1], out orderId))
                            {
                                Payment.AccessPay(orderId);
                        ...
                        //ShopUtils.Save_File(Server.MapPath("~/log/") + DateTime.Now.ToString("yyyyMMddHHmmss") + "-pg.txt", log, System.Text.Encoding.UTF8);
                        /////////////////////////////////////////////////////////////////////////////////////////////////////////////
                    }
                    else//验证失败
                    {
